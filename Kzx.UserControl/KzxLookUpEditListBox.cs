...
{
    public partial class KzxLookUpEditListBox : KzxBaseControl
    {
        private string _Value = string.Empty;

        public KzxLookUpEditListBox()
        {
            InitializeComponent();
        ...
        public string Columns
        {
            ...
            set
            {
                ...
                for (int i = 0; i < doc.DocumentElement.ChildNodes.Count; i++)
                {
                    ...
                    this.ValueControl.Properties.Columns.Add(info);
                }
                doc = null;
            }
        }
        ...
        /// Null值的显示信息
        /// </summary>
        [Category("外观"), Description("NullText,Null值的显示信息"), Browsable(true)]
        ...
        public TextEditStyles TextEditStyle
        {
            get
            {
                return this.ValueControl.Properties.TextEditStyle;
            }
            set
            {
        ...
        /// 勾选控件
        /// </summary>
        public LookUpEdit ContentControl
        {
            get
            {
        ...
        private void lookUpEdit_QueryPopUp(object sender, CancelEventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(filterstring) == false)
            {
                ...
                {
                    expressionarray = filterstring.Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
                    for (int i = 0; i < expressionarray.Length; i++)
                    {
                        sb.Clear();
                        if (string.IsNullOrWhiteSpace(expressionarray[i]) == false)
                        {
                            filterarray = expressionarray[i].Split(new string[] { "=", ">=", "<=", "<>", ">", "<" }, StringSplitOptions.RemoveEmptyEntries);
                            if (filterarray.Length >= 2)
                            {
                                for (int h = 0; h < 1; h++)
                                {
                                    ...
                                    valuearray = filterarray[1].Split(new char[] { '|' }, StringSplitOptions.RemoveEmptyEntries);
                                    if (valuearray.Length >= 2)
                                    {
                                        for (int cindex = 0; cindex < this.ObjectList.Count; cindex++)
                                        {
                                            if (this.ObjectList[cindex] is IControl)
                                            {
                                                if (((IControl)this.ObjectList[cindex]).Table.Equals(valuearray[0], StringComparison.OrdinalIgnoreCase) == true &&
                                                    ((IControl)this.ObjectList[cindex]).Field.Equals(valuearray[1], StringComparison.OrdinalIgnoreCase) == true)
                                                {
                                                    object v = ((IControl)this.ObjectList[cindex]).GetValue();
                                                    if (v != null)
                                                    {
                                                        sb.Append("'" + ((v == null || v == DBNull.Value) ? string.Empty : v.ToString()) + "'");
                                                        break;
                                                    }
                                                }
        ...
        private void lookUpEdit_Closed(object sender, DevExpress.XtraEditors.Controls.ClosedEventArgs e)
        {
            BindingSource ss = this.ValueControl.Properties.DataSource as BindingSource;
            if (ss != null)
            {
        ...
        private void LayoutControl()
        {
            ...
            this.ValueControl.Properties.TextEditStyle = TextEditStyles.Standard;
