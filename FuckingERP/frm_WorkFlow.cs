...
namespace FuckingERP
{
    public partial class frmWorkFlow : DockContent
    {
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void Btn_Post_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            //MessageBox.Show(dtDesign.Rows.Count.ToString());
            int iOrder = 0;
            //Control[] gc = new Control[iCount];
            foreach (Control c in TabControl2.SelectedTabPage.Controls)
            {
                //string sID = "";
                string sFrmName = "";
            ...
            //RequestDataArgs inArgs = new RequestDataArgs();
            //inArgs.UserId = "UserId";
            //inArgs.OperateId = 1;
            //inArgs.BillTypeId = "frm_DesignFlowMap";
            //inArgs.ClientId = "ClientId";
            //ResponseDataArgs outArgs = null;

            //int ret = YZCommon.SysVar.YZERPDataProvider.DesignerSave(inArgs, ref MainClass.dsFlowMap, out outArgs);
            ...
            ////加载流程图数据
            //MainClass.dsFlowMap = YZCommon.SysVar.YZERPDataProvider.LoadBill(inArgs, "Sys_FlowMap", MainClass.sFlowMap, out outArgs);
            //MainClass.flowMapDV.Table = dtDesign;
                ...
                else
                {
                    ...
                    foreach (DataRow dr in flowMapDV.Table.Rows)
                    {
                        ...
                        else if (Equals(dr["sType"].ToString().ToUpper(), "Label".ToUpper())) //如果是Label，则创建Label
                        {
                            ...
                            lbl.Text = dr["sCaption"].ToString(); //YZCommon.sysClass.ssLoadMsg(dr["sMsgID"].ToString());   
